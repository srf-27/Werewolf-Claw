...
- **纯规则模式**：不联网、不调模型也能跑完整流程，方便验证和演示(很痴呆)
...
1. 打开 `/game`，选择板子（当前废除，因为数据集只有9人局，只推荐内置 9 人标准板）和模型配置，或勾掉「用模型」纯规则跑。
2. 点击开局，10 个 Agent（1 法官 + 9 玩家，按板子人数）在后台线程跑对局，页面轮询刷新大屏。
3. 顶栏可以暂停 / 继续 / 调倍速；座位卡上点「接管」由人来替这个 Agent 行动，「退出」后换回模型继续。人接管时会出现「助手」按钮， 点击后出现浮窗，可以帮助决策。
4. 切换「上帝视角」可看身份徽章和狼频道私聊；对局可导出 JSON。


模型调用失败时该次动作自动退回规则版，对局不会中断。
...
│   ├── agents/                # Agent 编排层：每个 Agent 一条 Node + Flow，通过具体实现 core/node.py 中的类完成
...
│   │   └── standard12_board/  #   12 人标准板：4 狼 4 民 + 预女猎守（目前废除）
