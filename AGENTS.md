...
- `agents/`：Agent 层。把 `core/` 的能力编排成一条 Node + Flow 的流程，每个 Agent 一条 Flow（`chatagent`、`judge_agent`、`player_agent`）。这里只做编排，不放具体能力。
...
判断标准：能被多个入口（Web、命令行、脚本）复用、且与传输方式无关的逻辑，属于 `core/`；把 `core/` 的能力串成一条 Node + Flow 的流程，属于 `agents/`；只服务于某个具体入口的，放 `app/`。
