...
REVISE_PROMPT = """上面这个问题已经回答过 {round} 次，之前的回答是：
...
def _complete(memory: Memory, context: list[dict[str, Any]]) -> tuple[str, Any]:
    """调用模型，失败时抛 ModelError。"""
    try:
        completion = chat_full(context, temperature=0.7)
...
def send(memory: Memory, text: str, quote: Quote | None = None) -> TurnResult:
    ...
    quote_extra = _resolve_quote(memory, quote)
    remembered = memory.add_user(content, quote_extra)
    user, _ = memory.last_exchange()
    user_seq = user["seq"] if user else None

    reply, usage = _complete(memory, memory.build_context(SYSTEM_PROMPT))

    # 等待期间如果这条消息被编辑或撤回了，就把这次回复丢掉，不写进记忆
    current_user, _ = memory.last_exchange()
    if user_seq is None or current_user is None or current_user["seq"] != user_seq:
...
def edit_last(memory: Memory, text: str) -> TurnResult:
    ...
    if not content:
        raise InvalidRequest("消息内容不能为空")
    if memory.truncate_last_exchange() == 0:
        raise ConflictError("没有可编辑的消息")
    return send(memory, content)
...
def regenerate(memory: Memory) -> TurnResult:
    ...
    raw, _ = _complete(memory, context)
    new_reply, shortcoming = split_revision(raw)
